-- ============================================================
-- URGENT FIX: performance_dashboard_rpc returning 500
-- ============================================================
-- تطبّق هذا مباشرة في Supabase Dashboard > SQL Editor
-- اللصق والتشغيل بدون تعديل
-- ============================================================

-- 1. تأكد من صلاحيات الـ views
...
-- 2. أعد بناء الـ RPC مع إصلاح كامل وshape صحيح للـ frontend
  ...
  RETURN (
    ...
    alerts_source AS (
      SELECT
        ...
        CASE
          WHEN cr.total_orders = 0 THEN 'critical'
          WHEN cr.total_orders < COALESCE(ta.avg_total_orders, 0) * 0.5 THEN 'warning'
          ELSE 'info'
        END AS severity,
        CASE
          WHEN cr.total_orders = 0 THEN 1
          WHEN cr.total_orders < COALESCE(ta.avg_total_orders, 0) * 0.5 THEN 2
          ELSE 3
      ...
      FROM current_ranked AS cr
      CROSS JOIN team_avg AS ta
      WHERE cr.total_orders < COALESCE(ta.avg_total_orders, 0) * 0.8
         OR cr.total_orders = 0
    )
...
-- 4. إعلام PostgREST بإعادة تحميل الـ schema
