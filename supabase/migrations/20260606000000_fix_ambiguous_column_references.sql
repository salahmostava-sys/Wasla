...
--
-- 1. public.calculate_employee_salary (20260411020000)
--    Line 141: `WHERE employee_id = p_employee_id` on public.daily_shifts
--    Ambiguous because RETURNS TABLE declares `employee_id UUID`.
--    Fix: qualify with table alias `ds.employee_id`.
--
-- 2. public.preview_salary_for_month (20260416000001)
--    Line 114: `WHERE employee_id = v_emp.id` on daily_shifts
--    Ambiguous because RETURNS TABLE declares `employee_id UUID`.
--    Fix: qualify with table alias `ds.employee_id`.
--
-- 3. public.preview_salary_for_month_v2 (20260416000000)
--    Warning: variable `c_days_per_month` declared but never used.
  ...
  LOOP
    ...
    ELSIF v_app.work_type = _const_work_hybrid() THEN
      ...
      ELSE
        ...
        LOOP
          -- FIX: qualify column with table alias `ds.` to avoid ambiguity
          -- with the RETURNS TABLE column `employee_id`
      ...
      ELSIF v_app.work_type = c_hybrid THEN
        ...
        ELSE
          FOR v_day IN SELECT generate_series(v_start, v_end, '1 day'::interval)::date AS day_date LOOP
            -- FIX: qualify column with alias `ds.` to avoid ambiguity
            -- with the RETURNS TABLE column `employee_id`
            SELECT ds.hours_worked INTO v_hours_worked
