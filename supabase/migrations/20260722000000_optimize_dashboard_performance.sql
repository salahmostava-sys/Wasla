-- Migration: Optimize dashboard performance with Materialized Views and Composite Indexes
-- Objective: Drastically reduce Postgres CPU consumption and latency (< 100ms)

-- 1. Composite indexes on daily_orders to speed up platform & daily aggregation
CREATE INDEX IF NOT EXISTS idx_daily_orders_perf_lookup
  ON public.daily_orders (employee_id, date, app_id)
  INCLUDE (orders_count)
  WHERE (status IS NULL OR status <> 'cancelled');

CREATE INDEX IF NOT EXISTS idx_daily_orders_date_orders
  ON public.daily_orders (date, orders_count)
  WHERE (status IS NULL OR status <> 'cancelled');

-- 2. Materialized View for rider daily performance
CREATE MATERIALIZED VIEW IF NOT EXISTS public.mv_rider_daily_performance AS
SELECT
  p.employee_id,
  p.employee_name,
  p.city,
...
-- Unique index required for CONCURRENTLY refresh without locking tables
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_rider_daily_perf_pk
  ON public.mv_rider_daily_performance (employee_id, date);

CREATE INDEX IF NOT EXISTS idx_mv_rider_daily_perf_date
  ON public.mv_rider_daily_performance (date);

CREATE INDEX IF NOT EXISTS idx_mv_rider_daily_perf_orders
  ON public.mv_rider_daily_performance (total_orders DESC);

-- 3. Concurrent Refresh function
...
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_rider_daily_performance;
END;
$$;

-- Grant permissions for authenticated and service_role
GRANT EXECUTE ON FUNCTION public.refresh_dashboard_materialized_views() TO authenticated, service_role;
GRANT SELECT ON public.mv_rider_daily_performance TO authenticated, service_role;

-- 4. Update v_rider_monthly_performance to read from mv_rider_daily_performance
...
WITH monthly_base AS (
  ...
  FROM public.mv_rider_daily_performance AS d
  GROUP BY
    d.employee_id,
    d.employee_name,
    d.city,
...
-- 5. Update performance_dashboard_rpc to read from mv_rider_daily_performance
...
BEGIN
  ...
  RETURN (
    ...
    leaderboard_date AS MATERIALIZED (
      ...
      FROM public.mv_rider_daily_performance
