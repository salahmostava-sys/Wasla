-- Re-create performance_dashboard_rpc to fix the employee status column reference
CREATE OR REPLACE FUNCTION private.performance_dashboard_rpc(
  p_month_year TEXT,
  ...
  RETURN (
    ...
    SELECT jsonb_build_object(
      'summary', jsonb_build_object(
        ...
        'totalEmployees', (SELECT COUNT(*) FROM public.employees WHERE status = 'active'),
      ...
      'topRiderToday', (
        SELECT jsonb_build_object(
          'employeeId', cdr.employee_id,
          'employeeName', cdr.employee_name,
          'totalOrders', cdr.total_orders
        )
        FROM current_day_ranked cdr
        WHERE top_rank = 1
        LIMIT 1
      ),
      'lowestRiderToday', (
      ...
      'ordersByApp', COALESCE((
        SELECT jsonb_agg(row_to_json(oba))
        FROM app_comparison oba
      ), '[]'::jsonb),
      'ordersByCity', COALESCE((
        SELECT jsonb_agg(row_to_json(obc))
      ...
      'monthlyTrend', COALESCE((
        SELECT jsonb_agg(row_to_json(mt))
        FROM monthly_trend mt
      ), '[]'::jsonb),
      'alerts', COALESCE((
        SELECT jsonb_agg(row_to_json(a))
        FROM (
          SELECT * FROM alerts_source
          ORDER BY severity_rank ASC, total_orders DESC
        ) a
      ), '[]'::jsonb)
    )
  );
