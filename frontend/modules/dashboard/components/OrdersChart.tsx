  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-border/60 bg-muted/20 px-4 py-3 flex flex-wrap items-baseline justify-between gap-2">
        ...
        <p className="text-2xl font-black tabular-nums">{loading ? '—' : totalOrders.toLocaleString('en-US')}</p>
      </div>
      <div>
            ...
            {ordersByApp.map((a) => {
              ...
              return (
                <div key={a.app} className="bg-card rounded-2xl p-4 shadow-card">
                  ...
                  <p className="text-2xl font-black">{a.orders.toLocaleString('en-US')}</p>
                  <p className="text-[10px] text-muted-foreground">هدف: {a.target.toLocaleString('en-US')} | {a.riders} مندوب</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {ordersByCity.map((c) => (
            <div key={c.city} className="rounded-xl bg-muted/30 p-4">
              ...
              <p className="text-2xl font-black">{c.orders.toLocaleString('en-US')}</p>
            </div>
          ))}
        </div>
      </div>
